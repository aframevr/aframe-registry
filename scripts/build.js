...
var metadataFetched = Object.keys(REGISTRY.components).map(function processComponent (npmName) {
  var component = REGISTRY.components[npmName];

  // Go through each A-Frame version and build component metadata for that version.
  return Promise.all(
    AFRAME_VERSIONS.map(function processVersion (aframeVersion, index) {
      ...
      if (!component.versions[aframeVersion]) {
        for (var i = index - 1; i >= 0; i--) {
          ...
          if (OUTPUT[oldAFrameVersion].components[npmName]) {
            console.log(npmName, 'marked to fall back to', oldAFrameVersion, 'entry');
